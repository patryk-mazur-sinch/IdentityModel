...
{
    public class PremiumInfoExtensionsTests
    {
        ...
        {
            var document = File.ReadAllText(FileName.Create("success_premiuminfo_response.json"));
            var handler = new NetworkHandler(document, HttpStatusCode.OK);

            var client = new HttpClient(handler);
            var response = await client.PostPremiumInfoAsync(new PremiumInfoRequest
            {
                Address = Endpoint,
                Token = "token"
            });
        ...
        {
            var document = File.ReadAllText(FileName.Create("failure_premiuminfo_response.json"));
        ...
        public async Task Http_request_should_have_correct_format()
        {
            ...
            var client = new HttpClient(handler);
            var request = new PremiumInfoRequest()
            {
                Address = Endpoint,
                Token = "token"
            };
            ...
            var response = await client.PostPremiumInfoAsync(request);
        ...
        [Fact]
        public async Task Setting_request_body_should_have_correct_format()
        {
            var handler = new NetworkHandler(HttpStatusCode.NotFound, "not found");
            ...
            var response = await client.PostPremiumInfoAsync(new PremiumInfoRequest
            {
                Address = Endpoint,
                Token = "token",
                Data = new {CorrelationId = "123-321-321"}
            });

            var request = handler.Request;

            request.Headers.Should().Contain(h => h.Key == "Authorization" && h.Value.First() == "Bearer token");
